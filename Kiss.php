...
class Kiss{
	...
	function getPostsByTime($count=10,$start=0){
		$sql="select rowid,title,tag,description,posttime,postname from post where posttime<{$this->currentTime} order by posttime desc limit {$start},{$count}";
		return $this->getData($sql);
	}
	...
	*/
	...
	function getPostsByRand($count=10){
		...
		return $this->getData("select rowid,title,postname from post where rowid in (".substr($s_ids,1).") and posttime<{$this->currentTime}");
	}
	...
	function getPostByName($name){
		$sql="select * from post where postname='".$this->db->escapeString($name)."'";
